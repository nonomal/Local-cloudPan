...
// 创建上传中需要的文件夹
const tempBasePath = path.join(utools.getPath('userData'), 'temp');
const chunkDir = path.join(tempBasePath, 'chunktemp');
const fileInfoDir = path.join(tempBasePath, 'filetemp');

// 确保基础临时目录存在
fs.ensureDirSync(tempBasePath);
...
router.post('/merge', async (ctx) => {
  try {
    ...
    const uploadPath = path.join(config.publicPath, reqPath);
...
router.get('/checkFileList', async (ctx) => {
  try {
    ...
    const config = getConfig();
    reqPath = path.join(config.publicPath, reqPath);
...
router.get('/fileList', async (ctx) => {
  try {
    ...
    const config = getConfig();
    const reqFilePath = path.join(config.publicPath, reqPath);
    // 读取路径下所有文件
...
router.post('/rename', async (ctx) => {
  try {
    ...
    const config = getConfig();
    const oldPath = path.join(config.publicPath, reqPath, oldName);
    const newPath = path.join(config.publicPath, reqPath, newName);
...
router.post('/fileMoveOrCopy', async (ctx) => {
  try {
    ...
    for (const file of fileList) {
      const oldPath = path.join(config.publicPath, reqPath, file);
      const newPath = path.join(config.publicPath, destination, file);
...
router.post('/createDir', async (ctx) => {
  try {
    ...
    const config = getConfig();
    const dirPath = path.join(config.publicPath, reqPath, dirName);
...
router.get('/thumbnail/:path*', async (ctx) => {
  ...
  sharp.cache(false);
  const config = getConfig();
  const imagePath = path.join(config.publicPath, reqPath);
  try {
